...
> The agent and PTX share **one Oracle DB**. PTX versions prompts in
> `PTX_PROMPT_HIS` and flips `IS_ACTIVE`; the agent reads the active row. No
> cross-system sync job. Join key everywhere: **`NODE_NM`** (unique within the
> current flow).

## Agent-owned tables (structure FIXED — never altered by PTX)

| Agent table / column            | Used by PTX as                                  |
|---------------------------------|------------------------------------------------|
...
| `CHAT_VER_MAS.MAIN_MODEL_NM`    | **unused by PTX** — model selection happens inside the agent |
| `NODE_MAS.ID`                   | node id; FK target of `PTX_PROMPT_HIS.NODE_MAS_ID` |
...
PTX never changes the **structure** of these two tables; it only reads them.

## PTX-owned table the agent READS: `PTX_PROMPT_HIS`
...
the active row for each node to get both prompts. Other PTX-owned tables
(`PM_FLOW_VER` + `PM_FLOW_VER_NODE`, `PM_TEST_*`, `PM_RAGAS_*`, `PTX_AUDIT_HIS`) are
PTX-internal — the agent does not read them.
...
def load_node_prompts(node_nm: str) -> tuple[str, str]:
    ...
    with conn, conn.cursor() as cur:
        cur.execute(
            "SELECT v.SYSTEM_PROMPT, v.USER_PROMPT "
            "FROM PTX_PROMPT_HIS v JOIN NODE_MAS n ON n.ID = v.NODE_MAS_ID "
            "WHERE n.NODE_NM = :node_nm AND v.IS_ACTIVE = 'Y'",
            node_nm=node_nm,
        )
        row = cur.fetchone()
...
process start, which breaks A/B RAGAS. During an A/B run PTX **temporarily flips
...
Editing happens only in the PTX UI. **Activate** does, in one transaction:
1. flip `IS_ACTIVE` on `PTX_PROMPT_HIS` (one active per node),
2. cut a new `PM_FLOW_VER` snapshot (whole-flow version bumps; node versions kept).

The agent picks up the new prompt by reading the active `PTX_PROMPT_HIS` row.
