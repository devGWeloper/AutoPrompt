...
| 테이블 | 컬럼 | 민감도 |
|--------|------|--------|
| `PM_NODE_PROMPT_VER` | `SYSTEM_PROMPT`, `USER_PROMPT` | 높음 (지식재산) |
| `PM_TEST_CASE` | `INPUT_DATA`, `EXPECTED_OUTPUT` | 중간 (테스트 데이터) |
| `PM_RAGAS_RESULT` | `ANSWER`, `CONTEXTS`, `GROUND_TRUTH` | 중간 (모델 출력/컨텍스트) |
...
3. 암호화 테이블스페이스 생성 후 PM 테이블 이관:
   ...
   ALTER TABLE PM_NODE_PROMPT_VER MOVE TABLESPACE pm_enc;
   -- 인덱스 REBUILD 필요
   ```
...
```sql
ALTER TABLE PM_NODE_PROMPT_VER MODIFY (SYSTEM_PROMPT ENCRYPT USING 'AES256');
ALTER TABLE PM_NODE_PROMPT_VER MODIFY (USER_PROMPT  ENCRYPT USING 'AES256');
