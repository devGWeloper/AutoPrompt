...
- **`gaia`** — 두 번째 엔드포인트(gaia 게이트웨이)용. 이쪽은 JSON-RPC 필수 필드(`jsonrpc:"2.0"`, `id`, `method`=`rpcMethod`)를 **본문 필드와 같은 레벨에** 요구한다(없으면 `-32600 Invalid Request … Missed fields: method, jsonrpc, id`). 그래서 body 는 `{jsonrpc, id, method, query, user_id, session_id, gaia_session_name, gaia_input_channel, chat_type, a2a_remote_urls, is_super_agent, main_model_name, session_system_prompt, request_url, trace_id}` 평면 구조다. 질문은 `message`가 아니라 **`query`**, `gaia_input_channel`은 `"api"` 고정, `request_url`은 실제로 호출하는 URL, `trace_id`는 빈 문자열이다(실제 번호는 `session_system_prompt.TRACE_ID`). `params` 아래로 넣으면 게이트웨이가 그 객체를 통째로 핸들러 첫 인자(`query`)에 밀어넣어 나머지 필드가 전부 빈 값이 된다.
