...
> (`PM_TEST_RUN`/`PM_TEST_RESULT`/`PM_FLOW_VER`/`PM_FLOW_VER_NODE`)은 RAGAS 중심 정리 때 제거됐다.
...
# 스키마 생성 (마이그레이션 도구 없음 — 권위 스키마는 backend/sql/ddl_initial.sql).
#   PM_* 테이블만 생성, 운영 테이블 CHAT_VER_MAS/NODE_MAS/MODEL_MAS는 건드리지 않음(이미 존재 가정).
#   내부 운영 DB의 스키마 변경은 DBA가 리뷰한 SQL을 직접 적용한다.
python scripts/create_pm_tables.py     # 모델 기반 PM_* 생성 (--dry-run 으로 미리보기)
#   또는 backend/sql/ddl_initial.sql 을 직접 실행
